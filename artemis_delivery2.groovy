node {
	properties(
		...
		stage("Authenticate"){
			timestamps {
				ws {
					sh '''
						ssh centos@dev1.devops-chicago.com $(aws ecr get-login --no-include-email --region us-east-1)
						'''
				}
			}
		}
			...
			timestamps {
				ws {
					try {
						sh '''
							#!/bin/bash
							IMAGES=$(ssh centos@dev1.devops-chicago.com docker ps -aq) 
							for i in \$IMAGES; do
								ssh centos@dev1.devops-chicago.com docker stop \$i
								ssh centos@dev1.devops-chicago.com docker rm \$i
							done 
							'''
					} catch(e) {
	...
	stage("Run Container"){
		timestamps {
			ws {
				sh '''
					ssh centos@dev1.devops-chicago.com docker run -dti -p 5001:5000 945548589381.dkr.ecr.us-east-1.amazonaws.com/artemis:${Version}
					'''
            }
        }
    }
